...
def upsert_contest(contest: dict) -> bool:
    """Supabase external_contests 테이블에 upsert (지역 필터링 적용, URL 기준 중복 방지)"""
    ...
    try:
        ...
        logger.info(f"저장 성공: {contest.get('title', '')[:45]}... | 주최: {contest.get('organizer', 'N/A')}")
        return True
...
def deactivate_expired():
    """마감일 지난 공모전 비활성화 (is_active = false)"""
    today = get_kst_now().date().isoformat()
    url = f"{SUPABASE_URL}/rest/v1/external_contests?deadline=lt.{today}&is_active=eq.true"
    headers = {
        'apikey': SUPABASE_KEY,
        'Authorization': f'Bearer {SUPABASE_KEY}',
        'Content-Type': 'application/json'
    ...
    try:
        resp = requests.patch(url, json={'is_active': False}, headers=headers)
        resp.raise_for_status()
        logger.info("만료 공모전 비활성화 완료")
    except Exception as e:
        logger.error(f"만료 공모전 비활성화 오류: {e}")
                ...
                try:
                    ...
                    if date_el:
                        ...
                        if match:
                            ...
                            deadline = f"{end_year}-{end_month}-{end_day}"
                    ...
                    contest = {
                        'title': title,
                        'organizer': organizer,
                        'category': map_field(title, category_raw),
                        'deadline': deadline,
                        'url': detail_url,
                        'thumbnail_url': None,
                        'is_active': True,
