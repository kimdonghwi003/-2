...
export async function POST(request: NextRequest) {
  ...
  const { match_id, reviewee_id, rating } = await request.json()
  ...
  const { data: match } = await supabase.from('matches').select('author_id').eq('id', match_id).single()
  const finalRevieweeId = reviewee_id || (match?.author_id !== user.id ? match?.author_id : null)
